...
if args.det_signal:
    ...
    if args.bwm_search:
        ...
        if args.bwm_model_select:
            pmin = np.append(pmin,-0.5)
    if args.eph_quadratic:
        pmin = np.append(pmin,-1e-11*np.ones(9)) # amps
...
if args.det_signal:
    ...
    if args.bwm_search:
        ...
        if args.bwm_model_select:
            pmax = np.append(pmax,1.5)
    if args.eph_quadratic:
        pmax = np.append(pmax,1e-11*np.ones(9)) # amps
...
if args.sampler == 'mnest':
    ...
    if args.det_signal:
        ...
        if args.bwm_search:
            ...
            if args.bwm_model_select:
                x0 = np.append(x0,0.4)
        if args.eph_quadratic:
            x0 = np.append(x0,np.random.uniform(-1e-11,1e-11,9))
    ...
    if args.det_signal:
        ...
        if args.bwm_search:
            ...
            if args.bwm_model_select:
                ...
                param_ephquad += 1
        if args.eph_quadratic:
            cov_diag = np.append(cov_diag,1e-23*np.ones(9))
    ...
    if args.det_signal and args.eph_quadratic:
        cov_diag[param_ephquad:param_ephquad+9,param_ephquad:param_ephquad+9] = 0.5 * ephem_fisher / ephem_norm**2.0
